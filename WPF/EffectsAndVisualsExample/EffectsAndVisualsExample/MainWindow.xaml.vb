Class MainWindow
	...
	Private Sub drawingSurface_MouseLeftButtonDown(ByVal sender As Object, ByVal e As MouseButtonEventArgs)
		...
		ElseIf cmdSelectMove.IsChecked = True Then
			...
			If Not visual Is Nothing Then
				...
				' An alternate solution would be to store the top-left point of every visual in a collection in the 
				...
				If selectedVisual IsNot Nothing AndAlso selectedVisual IsNot visual Then
					ClearSelection()	' The selection has changed. Clear the previous selection.
				End If
	...
	Private Sub DrawSquare(ByVal visual As DrawingVisual, ByVal topLeftHandCorner As Point, ByVal isSelected As Boolean)

		Using dc As DrawingContext = visual.RenderOpen()
			Dim currBrush As Brush = drawingBrush
			If isSelected Then
				currBrush = selectedDrawingBrush
			End If

			dc.DrawRectangle(currBrush, drawingPen, New Rect(topLeftHandCorner, squareSize))
		End Using

	End Sub

	''' <summary>
	''' Draw dashed box around selection
	''' </summary>
